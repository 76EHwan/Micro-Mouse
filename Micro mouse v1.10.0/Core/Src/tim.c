...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim) {
	if (htim->Instance == TIM3) {
		...
		// 2. 오프셋 보정 (캘리브레이션 된 값 빼기)
		float gyro_z_dps = imu_data.Gyro_Z - imu_data.Gyro_Z_Offset;

		// 3. 노이즈 필터링 (너무 작은 값은 무시, 데드존 설정)
		if (gyro_z_dps > -0.5f && gyro_z_dps < 0.5f) {
			gyro_z_dps = 0.0f;
		}

		// 4. 각도 적분 (적분 식: 현재 각도 += 각속도 * 시간변화량)
		// dt = 0.001 (1ms 타이머라고 가정 시)
		// 1kHz(1ms)라면 0.001을 곱해줍니다. TIM6 설정에 맞춰 수정하세요.
		imu_data.Yaw_Angle += gyro_z_dps * dt;

		// 5. 0~360도 범위 유지 (선택 사항)
