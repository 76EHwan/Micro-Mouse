...
uint8_t LSM6DS3_Init(I2C_HandleTypeDef *hi2c) {
    ...
    // 1. WHO_AM_I 레지스터 확인
    if (HAL_I2C_Mem_Read(hi2c, LSM6DS3_ADDR, REG_WHO_AM_I, I2C_MEMADD_SIZE_8BIT, &chipID, 1, 100) != HAL_OK) {
        return 1; // 통신 에러
    }
    ...
    data = 0x40;
    HAL_I2C_Mem_Write(hi2c, LSM6DS3_ADDR, REG_CTRL1_XL, I2C_MEMADD_SIZE_8BIT, &data, 1, 100);

    // 3. 자이로스코프 설정 (CTRL2_G)
    // ODR = 104Hz, FS = 2000dps
    // 0100 (104Hz) 11 (2000dps) 00 -> 0x4C
    data = 0x4C;
    HAL_I2C_Mem_Write(hi2c, LSM6DS3_ADDR, REG_CTRL2_G, I2C_MEMADD_SIZE_8BIT, &data, 1, 100);
...
void LSM6DS3_ReadAccel(I2C_HandleTypeDef *hi2c, LSM6DS3_Data_t *data) {
    ...
    HAL_I2C_Mem_Read(hi2c, LSM6DS3_ADDR, REG_OUTX_L_XL, I2C_MEMADD_SIZE_8BIT, rawData, 6, 100);
...
void LSM6DS3_ReadGyro(I2C_HandleTypeDef *hi2c, LSM6DS3_Data_t *data) {
    ...
    // 자이로 레지스터 6바이트 연속 읽기
    HAL_I2C_Mem_Read(hi2c, LSM6DS3_ADDR, REG_OUTX_L_G, I2C_MEMADD_SIZE_8BIT, rawData, 6, 100);
...
void LSM6DS3_ReadAll(I2C_HandleTypeDef *hi2c, LSM6DS3_Data_t *data) {
    ...
    HAL_I2C_Mem_Read(hi2c, LSM6DS3_ADDR, REG_OUTX_L_G, I2C_MEMADD_SIZE_8BIT, rawData, 12, 100);
