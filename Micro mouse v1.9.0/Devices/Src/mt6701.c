...
/* USER CODE BEGIN 0 */
MT6701_Data_t encData;

void MT6701_ReadSSI(void) {
    uint8_t rxBuffer[3] = {0, 0, 0}; // 24비트(3바이트) 수신 버퍼
    uint32_t rawValue = 0;

    // 1. CS Low (통신 시작)
    HAL_GPIO_WritePin(ENC_R_CS_GPIO_Port, ENC_R_CS_Pin, GPIO_PIN_RESET);
    ...
    if (HAL_SPI_Receive(&hspi1, rxBuffer, 3, 10) == HAL_OK) {
        ...
        // 각도 변환 (0 ~ 360도)
        encData.angle_deg = (float)encData.raw_angle * 360.0f / 16384.0f;
    ...
    // 5. CS High (통신 종료)
    HAL_GPIO_WritePin(ENC_R_CS_GPIO_Port, ENC_R_CS_Pin, GPIO_PIN_SET);
